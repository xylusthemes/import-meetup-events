...
class Import_Meetup_Events_Aioec {
	...
	public function import_event( $centralize_array, $event_args ){
		...
		if ( ! is_wp_error( $inserted_event_id ) ) {
			...
			$event_count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$this->event_instances_table} WHERE `post_id` = %d", absint( $inserted_event_id ) ) );
			...
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.SchemaChange
			$event_exist_count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$this->event_db_table} WHERE `post_id` = %d", absint( $inserted_event_id ) ) );
