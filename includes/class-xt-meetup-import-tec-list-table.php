 ...
 */
class XT_Meetup_Import_Tec_List_Table extends WP_List_Table {
	...
	public function __construct() {
		global $status, $page;
	        ...
	        parent::__construct( array(
	            'singular'  => 'xtmi_meetup_url',     // singular name of the listed records.
	            'plural'    => 'xtmi_meetup_urls',    // plural name of the listed records.
	...
	function column_title( $item ) {
		...
		// Build row actions.
		$actions = array(
		    'delete' => sprintf( '<a href="%1$s" onclick="return confirm(\'Warning!! Are you sure to Delete this Meetup url? Url will be permanatly deleted.\')">%2$s</a>',esc_url( wp_nonce_url( add_query_arg( $xtmi_url_delete_args ), 'xtmi_delete_url_nonce' ) ), esc_html__( 'Delete', 'xt-meetup-import' ) ),
		);
	...
	function column_action( $item ) {
		...
		$xtmi_run_import_args = array(
			'page'   => wp_unslash( $_REQUEST['page'] ),
			'xtmi_action' => 'xtmi_tec_run_import',
			'url_id'  => absint( $item['ID'] ),
		);

		// Return the title contents.
		return sprintf( '<a class="button-primary" href="%1$s">%2$s</a>',
			esc_url( wp_nonce_url( add_query_arg( $xtmi_run_import_args ), 'xtmi_run_import_nonce' ) ),
			esc_html__( 'Import Now', 'xt-meetup-import' )
		);
	}
	...
	function prepare_items() {
		...
		$data = $this->xtmi_get_meetup_url_data();
		if ( ! empty( $data ) ) {
	 ...
	 * @since    1.0.0
	 */
	function xtmi_get_meetup_url_data() {
		...
		if ( $url_query->have_posts() ) {
			while ( $url_query->have_posts() ) {
				...
				$url_id = get_the_ID();
				$url_terms = get_the_terms( $url_id, 'tribe_events_cat' );
				...
				$meetup_data['import_data'][] = array(
					'ID' => $url_id,
					'title' => get_the_title(),
					'xtmi_category' => implode( ', ', $url_term_names ),
				);
			}
		}
