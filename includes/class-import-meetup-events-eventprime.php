...
class Import_Meetup_Events_EventPrime {
	...
	public function import_event( $centralize_array, $event_args ) {
		...
		if ( ! is_wp_error( $inserted_event_id ) ) {
			...
			update_post_meta( $inserted_event_id, 'em_event_type', end( $ime_cats ) );
			...
			if ( $is_exitsing_event ) {
				do_action( 'ime_after_update_event_prime_' . $centralize_array['origin'] . '_event', $inserted_event_id, $centralize_array );
				return array(
					'status' => 'updated',
					'id'     => $inserted_event_id,
				);
			} else {
				do_action( 'ime_after_create_event_prime_' . $centralize_array['origin'] . '_event', $inserted_event_id, $centralize_array );
				return array(
					'status' => 'created',
					'id'     => $inserted_event_id,
				);
