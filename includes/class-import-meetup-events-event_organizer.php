...
class Import_Meetup_Events_Event_Organizer {
	...
	public function import_event( $centralize_array, $event_args ){
		...
		if ( ! is_wp_error( $inserted_event_id ) ) {
			...
			$edttable = $this->event_db_table;
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.SchemaChange
			$event_count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$edttable} WHERE `post_id` = %d", $inserted_event_id ) );
			...
			if( isset( $centralize_array['location']['name'] ) && $centralize_array['location']['name'] != '' ){
				...
				if( !empty( $loc_term_meta ) ){
					// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.SchemaChange
					$meta_keys = $wpdb->get_col( $wpdb->prepare( "SELECT `meta_key` FROM {$wpdb->prefix}eo_venuemeta WHERE `eo_venue_id` = %d", $loc_term_id ) );
					foreach ($loc_term_meta as $loc_value) {
						if( in_array( $loc_value['meta_key'], $meta_keys) ){ //phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key 
