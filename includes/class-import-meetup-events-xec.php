...
class Import_Meetup_Events_XEC {
	...
	public function create_event( $centralize_array = array(), $formated_args = array(), $event_args = array() ) {
        ...
        $event_content = $ime_events->htmltblock->convert( $event_content );
		...
		if ( $new_event_id && !is_wp_error( $new_event_id ) ) {
			...
			if ( $event_featured_image != '' ) {
				$ime_events->common->ime_set_feature_image_logic( $new_event_id, $event_featured_image, $event_args );
			}
			...
			do_action( 'ime_after_create_xec_' . $centralize_array['origin'] . '_event', $new_event_id, $formated_args, $centralize_array );
			return array(
				'status' => 'created',
				'id'     => $new_event_id,
			);
	...
	public function update_event( $event_id, $centralize_array, $formated_args = array(), $event_args = array() ) {
		...
		if ( $update_event_id && !is_wp_error( $update_event_id ) ) {
			...
			if( !empty( $allmetas ) ){
				...
				foreach( $allmetas as $key => $value ){
					if ( $ime_events->common->ime_is_updatable( $key ) ) {
			...
			$event_featured_image = $centralize_array['image_url'];
			if ( $event_featured_image != '' ) {
				$ime_events->common->ime_set_feature_image_logic( $update_event_id, $event_featured_image, $event_args );
			} else {
			...
			do_action( 'ime_after_update_xec_' . $centralize_array['origin'] . '_event', $update_event_id, $formated_args, $centralize_array );
			return array(
				'status' => 'updated',
				'id'     => $update_event_id,
			);
