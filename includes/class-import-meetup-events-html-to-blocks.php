<?php
/**
 * HTML to Gutenberg Blocks converter for Import Meetup Events.
 *
...
class Import_Meetup_Events_Html_To_Blocks {

	/**
	 * Convert HTML string to Gutenberg block markup.
	 *
	 * @since  1.0.0
	 * @param  string $html Raw HTML from Meetup API.
	...
	private function quote_block( string $inner ): string {
		...
		// If inner HTML already contains <p> tags (e.g. Meetup sends
	...
	private function embed_block( DOMNode $node ): string {
		...
		// Normalize: strip trailing slash Meetup sometimes adds.
		$src = rtrim( $src, '/' );
